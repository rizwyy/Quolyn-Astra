...
export const importFields=['active','pack_unit','sku','name','manufacturer_code','description','category','brand','price','price_unit','selling_unit','currency','tax_percent','cost','cost_unit','coverage','coverage_unit','stock_status','lead_time','aliases','attributes'] as const
export function validateImport(rows: Record<string,string>[], currency='INR') {
 ...
 const active=row.active?.trim().toLowerCase();if(active&&!['true','false','1','0','yes','no'].includes(active))errors.push('Active must be true or false')
