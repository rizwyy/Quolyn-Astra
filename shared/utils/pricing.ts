...
export interface PriceInput { quantity: string; unit: Unit; priceUnit: Unit; priceMinor: string; coverage?: string | null; coverageUnit?: string | null; billing?: 'requested' | 'delivered'; discountBps?: number | null; customerDiscountBps?: number; taxBps: number | null; costMinor?: string | null; costUnit?: string | null; included?: boolean }
export function convert(q: Decimal.Value, from: Unit, to: Unit): Decimal {
...
export function priceLine(x: PriceInput) {
  ...
  if (x.coverage && x.coverageUnit && x.unit !== 'box') {
    ...
    packs = convert(requested, x.unit, x.coverageUnit).div(coverage).ceil()
    delivered = packs.mul(coverage)
  }
  if (x.priceUnit === 'box' && x.unit !== 'box') {
    ...
    billable = convert(delivered, x.coverageUnit, x.priceUnit)
  } else billable = convert(requested, x.unit, x.priceUnit)
