alter table public.quolyn_products add column price_updated_at timestamptz not null default now();
update public.quolyn_products set price_updated_at=updated_at;
alter table public.quolyn_products add constraint attributes_object check(jsonb_typeof(attributes)='object');
alter table public.quolyn_enquiry_lines add constraint requirements_object check(jsonb_typeof(requirements)='object' and jsonb_typeof(missing)='array');
create or replace function quolyn_private.touch_product() returns trigger language plpgsql set search_path='' as $$ begin
new.updated_at:=now();
if new.price_minor is distinct from old.price_minor or new.price_unit is distinct from old.price_unit or new.currency is distinct from old.currency then new.price_updated_at:=now();else new.price_updated_at:=old.price_updated_at;end if;
return new;end $$;
create function quolyn_private.guard_line_review() returns trigger language plpgsql security definer set search_path='' as $$ declare p public.quolyn_products;k text;v text;actual text; begin
if TG_OP='INSERT' then new.corrections:=0;else new.corrections:=old.corrections+case when (new.description,new.quantity,new.unit,new.brand,new.sku,new.requirements) is distinct from (old.description,old.quantity,old.unit,old.brand,old.sku,old.requirements) then 1 else 0 end;end if;
if new.state='confirmed' and new.product_id is not null then
select * into p from public.quolyn_products where workspace_id=new.workspace_id and id=new.product_id;
if p.id is not null then
for k,v in select * from jsonb_each_text(new.requirements) loop
actual:=case when k='brand' then nullif(p.brand,'') else p.attributes->>k end;
if (actual is null or lower(actual)<>lower(v)) and length(trim(new.clarification))=0 then raise exception 'Record clarification for unknown or conflicting specification: %',k;end if;
end loop;
if coalesce(new.brand,'')<>'' and lower(new.brand)<>lower(p.brand) and length(trim(new.clarification))=0 then raise exception 'Record clarification for brand mismatch';end if;
end if;end if;
return new;end $$;
revoke all on function quolyn_private.guard_line_review() from public,anon,authenticated;
create trigger guard_line_review before insert or update on public.quolyn_enquiry_lines for each row execute function quolyn_private.guard_line_review();
...
if prod.price_updated_at<now()-interval '90 days' then warns:=warns||jsonb_build_array('Line '||pos||': price older than 90 days'); end if;
end if;
begin
...
if nullif(l->>'override_minor','') is not null then insert into public.quolyn_activity_logs(workspace_id,actor_id,entity_id,event,details) values(w,auth.uid(),qid,'pricing.override',jsonb_build_object('line',pos,'effective_price_minor',price::text,'discount_bps',disc));end if;
