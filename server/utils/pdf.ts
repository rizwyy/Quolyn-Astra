...
export async function renderQuotePdf(q:any,font:Buffer,logo?:Buffer):Promise<Buffer>{
 // PDFKit 0.20 accepts a Buffer font source; @types/pdfkit still narrows this option to string.
 const doc=new PDFDocument({size:'A4',margin:48,bufferPages:true,font:font as unknown as string}),chunks:Buffer[]=[]
 ...
 const range=doc.bufferedPageRange();for(let i=0;i<range.count;i++){doc.switchToPage(i);if(draft){doc.save().opacity(0.09).fillColor('#234e80').translate(180,410).rotate(-30).fontSize(74).text('DRAFT',0,0,{lineBreak:false});doc.restore()}doc.fontSize(8).fillColor('#8a9aaa').text(`${q.number} · R${q.revision}    ${draft?'DRAFT - subject to review':'Prepared quotation'}    |    Page ${i+1} of ${range.count}`,48,772,{width,align:'center',lineBreak:false})}
 doc.end();return done
}
