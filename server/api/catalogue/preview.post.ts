...
export default defineEventHandler(async event=>{
 ...
 const length=Number(getHeader(event,'content-length'));if(!Number.isFinite(length)||length<=0)throw createError({statusCode:411,message:'Upload must include a content length'});if(length>10551296)throw createError({statusCode:413,message:'Upload exceeds 10 MB'});
