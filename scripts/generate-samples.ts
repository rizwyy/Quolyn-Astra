...
const long={...q,total_minor:Number(q.total_minor)*35,snapshot:{...q.snapshot,lines:Array.from({length:35},(_,i)=>({...q.snapshot.lines[0],description:`${i+1}. Grey porcelain tile - fictional demonstration. `+'Extended product description, finish and dimensions checked by the salesperson. '.repeat(6)}))}}
